...
public:
	...
	/** 
	 * \brief Creates a File_handler object in a custom open mode
	 * @param name - name of the file
	 * @param opmode - open mode type (read, write, append, truncate, ...), https://en.cppreference.com/w/cpp/io/ios_base/openmode 
	 */
	File_handler(const std::string name, std::ios_base::openmode opmode) : fname(name), mode(opmode) 
		{ open_file(); }

	/** 
	 * \brief Creates a File_handler object in a default (read, write) open mode
	 * \details It is an error if the file doesn't exist
	 * @param name - name of the file
	 */
	File_handler(const std::string name) : 
		File_handler(name, std::ios_base::in | std::ios_base::out) { }	
