...
bool random_shuffle_test_double();
bool random_shuffle_test_str();
...
int main()
{
	...
	test_pass(random_shuffle_test(), "Random shuffling for ints");
	test_pass(random_shuffle_test_double(), "Random shuffling for Doubles");
	test_pass(random_shuffle_test_str(), "Random Shuffling for Strings");
}
...
{
	RNG rng;
	std::vector<double> v2s = {0.11, 0.21, 0.31, 0.41, 0.51, 0.61, 0.71, 0.81, 0.91, 1.01};
	std::vector<double> v_orig(v2s);
	rng.vector_shuffle<double>(v2s);
	return !is_equal_floats<double>({v2s}, {v_orig}, 0.01);
}

bool random_shuffle_test_str()
{
	RNG rng;
	std::vector<std::string> v2s = {"hunt", "lion", "try", "france", "recall", "begin", "is", "me"};
	std::vector<std::string> v_orig(v2s);
	rng.vector_shuffle<std::string>(v2s);
	return !(v2s == v_orig);
}
