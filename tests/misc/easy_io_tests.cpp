...
/*************************************************************** 
 * Suite for testing Easy_IO class for I/O operations 
 **************************************************************/
...
template <typename T> 
bool write_read_test(const vec2D<T>, const Easy_IO&, const std::function<bool(vec2D<T>, vec2D<T>)>);

template <typename T> 
bool write_read_test(const vec1D<T>, const Easy_IO&, const std::function<bool(vec2D<T>, vec2D<T>)>);
...
int main()
{
	test_pass(read_test_suite(), "Easy_IO read only");	
	test_pass(custom_test_suite(), "Easy_IO write and read custom properties");
	test_pass(default_test_suite(), "Easy_IO default properties");
	test_pass(write_1D_vector_test_suite(), "Easy_IO write and read for 1D vectors");
	test_pass(write_object_test_suite(), "Easy_IO for classes with ostream overload");
}
...
bool custom_test_suite()
{
	...
	// bool
	Easy_IO io_bool("./test_data/wr_bool.txt");
	std::function<bool(vec2D<bool>, vec2D<bool>)> is_equal_bool = is_equal_exact<bool>;
	...
	// int
 	Easy_IO io_int("./test_data/wr_int.txt");
	std::function<bool(vec2D<int>, vec2D<int>)>is_equal_int = is_equal_exact<int>;
	...
	// std::string
 	Easy_IO io_str("./test_data/wr_string.txt");
	std::function<bool(vec2D<std::string>, vec2D<std::string>)>is_equal_str = is_equal_exact<std::string>;
	...
	std::function<bool(vec2D<double>, vec2D<double>)>is_equal_double = is_equal_wrap;
	//
 	Easy_IO io_dbl("./test_data/wr_double.txt");
...
bool write_1D_vector_test_suite()
{
	...
	// bool
	Easy_IO io_bool("./test_data/wr_bool_1D.txt");
	std::function<bool(vec2D<bool>, vec2D<bool>)> is_equal_bool = is_equal_exact<bool>;
	...
	// int
 	Easy_IO io_int("./test_data/wr_int_1D.txt");
	std::function<bool(vec2D<int>, vec2D<int>)>is_equal_int = is_equal_exact<int>;
	...
	// std::string
 	Easy_IO io_str("./test_data/wr_string_1D.txt");
	std::function<bool(vec2D<std::string>, vec2D<std::string>)>is_equal_str = is_equal_exact<std::string>;
	...
	std::function<bool(vec2D<double>, vec2D<double>)>is_equal_double = is_equal_wrap;
	//
 	Easy_IO io_dbl("./test_data/wr_double_1D.txt");
...
bool write_object_test_suite()
{
	Easy_IO easy_io("./test_data/wr_struct.txt");
	TestStruct test_struct;

	// Write to file	
	easy_io.write_object(test_struct);
...
/** 
 * \brief Checks writing and reading under different Easy_IO settings
 ...
 */
template <typename T> 
bool write_read_test(const vec2D<T> orig_vec, const Easy_IO& io_obj, 
	const std::function<bool(vec2D<T>, vec2D<T>)> is_equal)
{
	// Write then read
	vec2D<T> new_vec;
...
/** 
 * \brief Checks writing and reading under different Easy_IO settings - 1D version
 * \details Writes to and then reads from the same file, compares 
 * 		the read 2D vector of type T to expected
 ...
 */
template <typename T> 
bool write_read_test(const vec1D<T> orig_vec, const Easy_IO& io_obj, 
	const std::function<bool(vec2D<T>, vec2D<T>)> is_equal)
{
	// Write then read
	vec2D<T> new_vec;
