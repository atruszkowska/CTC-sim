...
/*************************************************************** 
 * Suite for testing File_handler class for I/O operations 
 **************************************************************/
...
/**
 * \brief Testing correctness of File_handler default mode
 * \details Uses File_handler to write and then read
 * a line of text - compares both, true if identical
 */ 
bool default_io_test()
{
	// The default File_handler is both in and out and
	// truncates
...
/**
 * \brief Testing correctness of File_handler custom modes
 * \details Uses File_handler to write, read, append, and read again
 * 	compares reads to expected strings, true if identical. Important -
 * 	ensure the test file is deleted before the test.
 */ 
bool custom_io_test()
{
	// Custom File_handlers - in and out in append mode
	// appends
	std::string fname("./test_data/custom_mode.txt");
	File_handler out_file(fname, std::ios_base::out | std::ios_base::app);
	File_handler in_file(fname, std::ios_base::in);
