...
#target_times_min = [0, 0.5, 6, 30, 60]
target_times_min = [0.5, 6]

output_dir = 'lift_coefficient_cluster_snapshots'
...
# --- Find Valid File Pairs (base, Single_cell_lift_coefficient, Cell_cluster_lift_coefficient) ---

results_files = {}

# Matches strictly:
# 1. results<wss>_base.txt
# 2. results<wss>_Single_cell_lift_coefficient_<val>.txt
# 3. results<wss>_Cell_cluster_lift_coefficient_<val>.txt
pattern = re.compile(
    r'^results(0|2|5|20)_(base|(?:Single_cell_lift_coefficient|Cell_cluster_lift_coefficient)_[0-9eE\.\-]+)\.txt$'
)
...
for filename in os.listdir(data_dir):
    ...
    # Base case uses Np = 300 default unless specified in simulation parameters
    np_val = 300
...
for (wss, suffix), (pos_file, Np) in sorted(results_files.items()):
    ...
    if not os.path.exists(bond_path):
        print(f"Skipping WSS = {wss} | {suffix}: missing {bond_file}")
        continue

    print(f"\nProcessing WSS = {wss} dyn/cm^2 | Suffix: {suffix} (Np = {Np})")
    ...
    print(f"--- Key Realizations for WSS = {wss} | {suffix} ---")
    ...
    for cat_name, ri in key_realizations.items():
        ...
        for target_m in target_times_min:
            ...
            title = f'WSS {wss} | {suffix} | Case: {cat_name.upper()} (RI {ri}) | Time: {target_m} min ({actual_m:.1f} min)'
            out_filename = os.path.join(
                output_dir,
                f'snapshot_tau_{wss}_{suffix}_{cat_name}_ri_{ri}_t_{target_m}min.png'
            )

            generate_snapshot(df_step, clusters_for_step, title, out_filename)

print("\nLift coefficient cluster snapshots rendered successfully!")
