...
# --- Target Suffixes (Lower Normal Stiffness Only) -----------------------

target_suffixes = [
    'Bond_normal_stiffness_1.4e-05',
    'Bond_normal_stiffness_1.4e-5'
]
...
for wss in tau_values:
    ...
    # Resolve filename variation (e-05 vs e-5)
    for suffix in target_suffixes:
        p_path = os.path.join(data_dir, f'results{wss}_{suffix}.txt')
        b_path = os.path.join(data_dir, f'bonds{wss}_{suffix}.txt')
    ...
    if not pos_path or not bond_path:
        print(f"Skipping WSS = {wss}: missing files for lower normal stiffness (1.4e-05 / 1.4e-5)")
        continue

    print(f"\nProcessing WSS = {wss} dyn/cm^2 | Suffix: {matched_suffix}")
    ...
    if not final_ratios:
        print(f"No valid realizations found for WSS = {wss} | {matched_suffix}")
        continue

    # 5. Identify key realizations
    ...
    for cat_name, ri in key_realizations.items():
        ...
        for target_m in target_times_min:
            ...
            title = f'WSS {wss} | {matched_suffix} | Case: {cat_name.upper()} (RI {ri}) | Time: {target_m} min ({actual_m:.1f} min)'
            out_filename = os.path.join(
                output_dir,
                f'snapshot_tau_{wss}_{matched_suffix}_{cat_name}_ri_{ri}_t_{target_m}min.png'
            )

            generate_snapshot(df_step, clusters_for_step, title, out_filename)

print("\nSnapshots rendered successfully!")
