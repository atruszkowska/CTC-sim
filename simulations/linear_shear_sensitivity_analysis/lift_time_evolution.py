...
for filename in os.listdir(data_dir):
    ...
    # Base case uses Np = 300 default unless specified
    np_val = 300
...
for (wss, suffix), (pos_file, Np) in sorted(results_files.items()):
    ...
    # Save ensemble averages with updated 'avg_lift_' prefix
    with open(f'avg_lift_numbers_{wss}_{suffix}.txt', 'w') as f:
        for t, value in zip(times, avg_series['number']):
            f.write(f'{t} {value}\n')

    with open(f'avg_lift_sizes_{wss}_{suffix}.txt', 'w') as f:
        for t, value in zip(times, avg_series['size']):
            f.write(f'{t} {value}\n')

    with open(f'avg_lift_ratios_{wss}_{suffix}.txt', 'w') as f:
    ...
    for var_key, config in variables_config.items():
        ...
        filename = f'tau_{wss}_lift_{suffix}_{config["filename_suffix"]}_time_evolution.svg'
