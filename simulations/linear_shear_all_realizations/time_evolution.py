...
# Define TAU dataset values and cell size parameter
tau_values = [0, 2, 5, 20]
Np = 300
...
for tau in tau_values:
    ...
    with open(f'avg_numbers_{tau}.txt', 'w') as f:
        for t, value in zip(times, avg_series['number']):
            f.write(f'{t} {value}\n')

    with open(f'avg_sizes_{tau}.txt', 'w') as f:
        for t, value in zip(times, avg_series['size']):
            f.write(f'{t} {value}\n')

    with open(f'avg_ratios_{tau}.txt', 'w') as f:
    ...
    for var_key, config in variables_config.items():
        ...
        #plt.title(f'TAU {tau} — {ylabel} Evolution', pad=12)
        plt.tight_layout()

        filename = f'tau_{tau}_{config["filename_suffix"]}_time_evolution.svg'
