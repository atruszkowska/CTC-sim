...
void Wall_bond::add_displacement_increments(const std::unique_ptr<Particle>& particle_i, RNG& rng, const double dist, const double delta_t)
{
    ...
    const double xj = wall_x;
    const double yj = wall_y;
	...
	// Normal displacement increments for the shear
...
// Check for failure
bool Wall_bond::bond_failed(bool& failed_in_shear, bool& failed_in_tension)
{
	// Default
...
void Wall_bond::correct_particle_positions_and_headings(std::unique_ptr<Particle>& particle_i, 
					const double delta_t)
{
	// Resultant on the bond directed from particle i to wall
	...
	// Adjust coordinates 
	particle_i->store_wall_bond_contribution_x((delta_t*delta_t)*F_tot_x/mass_i);
	particle_i->store_wall_bond_contribution_y((delta_t*delta_t)*F_tot_y/mass_i);

 	// Adjust heading
	particle_i->store_wall_bond_contribution_heading((delta_t*delta_t)*M_b/(0.5*mass_i*R_bond*R_bond));
}
